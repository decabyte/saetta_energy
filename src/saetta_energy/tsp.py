...
# def tsp_build_problem(cities, distances):
#     if HAS_GUROBI:
#         return gurobi_problem(cities, distances)
#     elif HAS_PULP:
#         return pulp_problem(cities, distances)
#     else:
#         raise Exception('No optimization library found!')


def gurobi_solve(cities, distances, output_flag=0, **kwargs):
    n = len(cities)
    ...
    for i in range(n):
        for j in range(i + 1):
            name = 'e{}_{}'.format(i, j)
            eVars[i, j] = model.addVar(obj=distances[i][j], vtype=GRB.BINARY, name=name)
            eVars[j, i] = eVars[i, j]

    for i in range(n):
        name = 'u{}'.format(i)
        uVars[i] = model.addVar(vtype=GRB.INTEGER, name=name)
...
if __name__ == '__main__':
    ...
    st = time.time()
    tsp_route, total_cost, model = gurobi_solve(cities, distances, output_flag=1)
    dt = time.time() - st

    #print('Problem Status: %s' % model.status)
